...
impl FileConfig {
    ...
    pub fn copy_file(template_path: &str, file_name: &str, destination: &Path) -> Result<()> {
        ...
        let file = PROJECT_DIR.get_file(&full_path)
            .ok_or_else(|| anyhow!("Source file '{}' not found in template directory", full_path))?;
